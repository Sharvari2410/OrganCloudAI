...
function DashboardPage() {
  ...
  const organDemand = data?.charts?.organDemand || [];
  const transplantStats = data?.charts?.transplantStats || [];
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <StatCard label="Total Donors" value={metrics.donors ?? 0} accent="from-brand-500 to-brand-700" />
        <StatCard label="Total Recipients" value={metrics.recipients ?? 0} accent="from-emerald-500 to-emerald-700" />
        <StatCard label="Active Requests" value={metrics.activeRequests ?? 0} accent="from-indigo-500 to-indigo-700" />
        <StatCard label="Organs Available" value={metrics.organsAvailable ?? 0} accent="from-cyan-500 to-blue-600" />
        <StatCard label="Emergency Cases" value={metrics.emergencyCases ?? 0} accent="from-rose-500 to-red-700" />
      </div>
